...
| `SegmentedXpBar`          | Multi-segment XP bar, one section per grade — `src/components/grade/SegmentedXpBar.tsx`. Its grade-label row and footer (`GradeLabelRow`/`XpBarFooter`, same directory) are shared with the podium's `EndGameXpProgress`, see `doc/game-views.md` — the bar-fill rendering itself is not shared since `EndGameXpProgress` needs a two-layer before/after diff animation |
| `HistoryRow`              | Expandable history row with participant table; the "Réponses" column renders `AnswerDots` (correct/incorrect/timeout counts derived from aggregate history data via `getAggregatedAnswerResults`, no chronological order)                                                                                                                                               |
